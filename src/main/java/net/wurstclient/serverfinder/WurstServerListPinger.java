...
public class WurstServerListPinger
{
	...
	private ArrayList<IServerFinderDisconnectListener> disconnectListeners =
		new ArrayList<>();
	private boolean notifiedDisconnectListeners = false;
	private boolean failedToConnect = true;
	
	public void addServerFinderDisconnectListener(
		IServerFinderDisconnectListener listener)
	{
	...
	public void add(final WurstServerInfo entry, final Runnable runnable)
		throws UnknownHostException
	{
		...
		final ClientConnection clientConnection = ClientConnection.connect(
			InetAddress.getByName(serverAddress.getAddress()),
		...
		clientConnection.setPacketListener(new ClientQueryPacketListener()
		{
			...
			public void onResponse(QueryResponseS2CPacket packet)
			{
				if(received)
					clientConnection.disconnect(
						new TranslatableText("multiplayer.status.unrequested"));
				else
				{
					received = true;
					...
					if(serverMetadata.getDescription() != null)
						entry.label =
							serverMetadata.getDescription().asString();
					else
						entry.label = "";
					...
					{
						entry.version =
							serverMetadata.getVersion().getGameVersion();
						entry.protocolVersion =
							serverMetadata.getVersion().getProtocolVersion();
					}else
					{
					...
					if(serverMetadata.getPlayers() != null)
					{
							...
							serverMetadata.getPlayers().getSample()))
						{
							...
							for(int var6 = 0; var6 < var5; ++var6)
							{
								...
								list.add(
									new LiteralText(gameProfile.getName()));
							}
							
							if(serverMetadata.getPlayers()
								.getSample().length < serverMetadata
									.getPlayers().getOnlinePlayerCount())
								list.add(new TranslatableText(
									"multiplayer.status.and_more",
									new Object[]{serverMetadata.getPlayers()
										.getOnlinePlayerCount()
										- serverMetadata.getPlayers()
											.getSample().length}));
					...
					clientConnection.send(new QueryPingC2SPacket(startTime));
					sentQuery = true;
					notifyDisconnectListeners();
				}
			}
			...
			public void onPong(QueryPongS2CPacket packet)
			{
				...
				entry.ping = m - l;
				clientConnection.disconnect(
					new TranslatableText("multiplayer.status.finished"));
			}
			...
			public void onDisconnected(Text reason)
			{
				if(!sentQuery)
				{
					WurstServerListPinger.LOGGER.error("Can't ping {}: {}",
						entry.address, reason.getString());
		...
		{
			clientConnection
				.send(new HandshakeC2SPacket(serverAddress.getAddress(),
					serverAddress.getPort(), NetworkState.STATUS));
						...
						new SimpleChannelInboundHandler<ByteBuf>()
						{
								...
								throws Exception
							{
								...
								try
								{
									...
									byteBuf.writeShort(7 + 2
										* serverAddress.getAddress().length());
									byteBuf.writeByte(127);
									cs = serverAddress.getAddress()
										.toCharArray();
									byteBuf.writeShort(cs.length);
									var4 = cs;
									var5 = cs.length;
									...
									channelHandlerContext.channel()
										.writeAndFlush(byteBuf).addListener(
								...
								ByteBuf byteBuf) throws Exception
							{
								...
								if(s == 255)
								{
									...
									String[] strings = Iterables.toArray(
										WurstServerListPinger.ZERO_SPLITTER
											.split(string),
										String.class);
									if("§1".equals(strings[0]))
									{
										MathHelper.parseInt(strings[1], 0);
										String string2 = strings[2];
										String string3 = strings[3];
										int j =
											MathHelper.parseInt(strings[4], -1);
										int k =
											MathHelper.parseInt(strings[5], -1);
										serverInfo.protocolVersion = -1;
										serverInfo.version = string2;
										serverInfo.label = string3;
										serverInfo.playerCountLabel =
											WurstServerListPinger
												.method_27647(j, k);
									}
								}
		...
		synchronized(clientConnections)
		{
			...
			while(iterator.hasNext())
			{
				...
				if(clientConnection.isOpen())
				{
					iterator.remove();
					clientConnection.disconnect(
						new TranslatableText("multiplayer.status.cancelled"));
				}
			}
